...
class PDUWorker(QObject):
    ...
    def __init__(self, config, parent=None):
        ...
        # 동시 Modbus 요청 방지를 위한 Lock 객체 생성
        self.modbus_lock = threading.Lock()
        # 단일 클라이언트 인스턴스 재사용 (소켓 고갈 방지)
    ...
    def stop_worker(self):
        if self.is_running:
            ...
            if hasattr(self, 'timer'):
                self.timer.stop()
            self.client.close() # 워커 종료 시 소켓 닫기
    ...
    def poll_data(self):
        ...
        # 제어 명령이 실행 중일 때는 폴링을 스킵하여 충돌 및 GUI 멈춤 방지
        if not self.modbus_lock.acquire(blocking=False):
            return
        ...
        try:
            ...
            rr_gv = self.client.read_input_registers(address=0, count=2, slave=MODBUS_SLAVE_ID)
            rr_gp = self.client.read_input_registers(address=200, count=1, slave=MODBUS_SLAVE_ID)
            ...
            rr_curr = self.client.read_input_registers(address=101, count=8, slave=MODBUS_SLAVE_ID)
            rr_watts = self.client.read_input_registers(address=201, count=8, slave=MODBUS_SLAVE_ID)
            rr_energy = self.client.read_input_registers(address=301, count=8, slave=MODBUS_SLAVE_ID)
            ...
            for i in range(8):
                port_num = i + 1
                try:
                    rr_state_single = self.client.read_coils(address=100 + port_num, count=1, slave=MODBUS_SLAVE_ID)
    ...
    def control_single_port(self, port_num, state):
        ...
        # 제어 시 Lock을 걸어 폴링과 통신 겹침을 원천 차단
        with self.modbus_lock:
            try:
                ...
                result = self.client.write_coil(address=address, value=state, slave=MODBUS_SLAVE_ID)
