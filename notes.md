...
* **HV Control**: CAEN 고전압(HV) 채널의 전원을 원격으로 켜고 끄거나, 전압/전류를 설정할 수 있습니다.
...
* <span style="color:red;">**SHUTDOWN ACTIVE**</span>: 무정전 전원 공급 장치의 축전 용량 임계값(15분) 이하로 떨어져, 시스템이 모든 HV 채널을 자동으로 종료하고 있습니다.
...
2.  **대기**: 시스템은 남은 시간이 15분 이하로 떨어지면 자동으로 POWER OFF 신호를 SY4527LC로 보냅니다. 별도의 조작 없이 대기합니다.
3.  **보고**: 방문 이전 기간에 발생한 정전에 대하여 실험 책임자 또는 관련 담당자에게 알립니다.
...
### 상황 3: 특정 HV 채널이 빨간색(Error)으로 변경
1.  **채널 비활성화**: `HV Control` 탭으로 이동하여 해당 슬롯과 채널 번호를 선택하고, **Power OFF** 버튼을 눌러 채널을 즉시 끕니다.
2.  **로그 확인**: `Log Viewer`에서 CAEN HV 관련 에러 메시지가 있는지 확인합니다.
